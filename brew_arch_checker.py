...
def print_formula_state(formula, architecture, length):
    ...
    if 'bottle' in formula and 'stable' in formula['bottle']:
        ...
        if architecture in bottles:
            print(f"✅ | {package_name:<{length}} | Available for {architecture}!")
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Checks your currently installed brew formulas for compatibility with certain architectures.')
    parser.add_argument('--architecture',type=str,default='arm64_big_sur',help='Architecture to look for (default: arm64_big_sur)', required=False)
